...
public class BltNeedle {
    ...
    public BltNeedle(Context context, Handler handler, BluetoothAdapter bluetoothAdapter, String address) {
        ...
        boolean connect = connect(mBluetoothDeviceAddress);
        if (connect){
            startWork();
        }
    }
    ...
    boolean isNeedTurnOff = false;
    public void turnOffDivices(){
        isNeedTurnOff = true;
        hot_state = false;
        motor_state =false;
        sendData();
    ...
    public void sendData() {
        ...
        if (/*hot_state && */!isNeedTurnOff) {
            SendBuf[2] |= 0x01;
        } else {
            SendBuf[2] &= ~0x01;
        }
        if (/*motor_state &&*/ !isNeedTurnOff) {
            SendBuf[2] |= 0x02;
        } else {
            SendBuf[2] &= ~0x02;
        }
